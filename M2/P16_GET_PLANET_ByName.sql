CREATE OR REPLACE PROCEDURE GET_PLANET_ByName(
id_jugador IN planet.player_id_player%TYPE, 
...
BEGIN
SELECT COUNT(player_id_player) INTO id_Encontrada
FROM PLANET
WHERE player_id_player = id_jugador;

IF id_Encontrada =0 THEN 
RAISE excepcion_Id;
END IF;

SELECT planet_Name, quantity_metal, quantity_crystal,
...
INTO
nombre, metal, cristal, deuterio, cazadores_L, cazadores_H,
...
FROM PLANET
WHERE id_Planet = id_jugador;

EXCEPTION

WHEN excepcion_Id THEN
DBMS_OUTPUT.PUT_LINE('No se ha encontrado este jugador en la base de datos');
...
id_jugador planet.player_id_player%TYPE;
